'''Check if parenthesis are balanced.
    ...
    This solution was developed by Erin Allard.
...
def isBalanced(s):
    '''Function returns NO/YES depending on whether or not
    brances are balanced
    '''
    braces_pairs = {
        '}': '{',
        ')':'(',
    ...
    if len(s)%2 != 0:
    	...
    	for i in range(len(s)):
    		if s[i] in braces_pairs.values():
    			my_stack.push(s[i])
    		else:
    			if braces_pairs[s[i]] == my_stack.peek():
    				my_stack.pop()
    			else:
    				return 'NO'
